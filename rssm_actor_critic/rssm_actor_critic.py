...
class RSSM(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, state_dim, hidden_dim, device):
        ...
        self.decoder = nn.Sequential(
            nn.Linear(state_dim + obs_dim + 1, hidden_dim),
            nn.ReLU(),
            nn.Linear(hidden_dim, state_dim),
            nn.ReLU(),
...
class Critic(nn.Module):
    ...
    def __init__(self, obs_dim, act_dim, hidden_dim, num_layers=1):
        ...
        layers = []
        layers.append(nn.Linear(obs_dim + 1, hidden_dim))
...
def main():
    ...
    critic = Critic(obs_dim, act_dim, hidden_dim, 16).to(device)
