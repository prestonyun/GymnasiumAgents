...
class ReplayBuffer:
    def __init__(self, directory, capacity=0, ongoing=False, minlen=1, maxlen=0):
        ...
        self._total_episodes, self._total_steps = self._count_episodes(directory)
    ...
    def dataset(self, batch, length):
        ...
        dataset = torch.utils.data.TensorDataset(*example)
    ...
    def _sample_sequence(self):
        ...
        sequence['is_first'] = np.zeros(len(sequence['action']), dtype=np.bool)
    ...
    def _enforce_limit(self):
        ...
        while self._loaded_episodes > 1 and self._loaded_steps > self._capacity:
            ...
            self._loaded_seteps -= eplen(episode)
            self._loaded_episodes -= 1
